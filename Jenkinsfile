    ...
    stages {
        ...
        stage('Code Quality Analysis') {
            steps {
                ...
                withSonarQubeEnv('SonarQube') {
                    sh 'npm run sonar-scanner'  // Run SonarQube for code quality
                }
            }
        }
        ...
        stage('Build Docker Image') {
            steps {
                ...
                sh "docker build -t $DOCKER_IMAGE_NAME:$DOCKER_IMAGE_TAG ."  // Build Docker image
            }
        }
        ...
        stage('Push to ACR') {
            steps {
                ...
                sh "docker login $ACR_LOGIN_SERVER -u $ACR_CREDENTIALS_USR -p $ACR_CREDENTIALS_PSW"  // Login to ACR
                sh "docker tag $DOCKER_IMAGE_NAME:$DOCKER_IMAGE_TAG $ACR_LOGIN_SERVER/$DOCKER_IMAGE_NAME:$DOCKER_IMAGE_TAG"  // Tag image
                sh "docker push $ACR_LOGIN_SERVER/$DOCKER_IMAGE_NAME:$DOCKER_IMAGE_TAG"  // Push image to ACR
            }
        }
        ...
        stage('Deploy to AKS') {
            steps {
                ...
                sh 'kubectl apply -f k8s/deployment.yaml --kubeconfig=$KUBECONFIG_PATH'  // Deploy to AKS
            }
        }
